...
all.equal(m_sorted, a_sorted) #TRUE -> use of cluster_SE_x_histo is verificated 


median_cluster_SE_1 <- apply(cluster_SE_1_histo, 2, median)
# Retained Intron 
...
sample_RI_2_histo <- apply(sample_RI_2, 1, function(x) {barplot(x, plot = TRUE, main="Random event of cluster 'Alternatively Spliced  - Retained Intron'", xlab='Amount of Bin', width=1)})
dev.off()

pdf("/nfs/home/students/a.schuhe/scripts/splicingREMs/plots/sample_RI_3_barplot.pdf")
sample_RI_3_histo <- apply(sample_RI_3, 1, function(x) {barplot(x, plot = TRUE, main="Random event of cluster 'Included - Retained Intron'", xlab='Amount of Bin', width=1)}) 
dev.off()
...
kmeans_plot_SE_k3 <- fviz_cluster(
  ...
  scale_color_manual(
    ...
    values = c('skyblue3', 'darkseagreen3', 'lightpink')) #+ theme(legend.position = 'none')
...
# skyblue3: Cluster 1 SE -> Included
# slateblue3: Cluster 2 SE -> Alternatively Spliced
# lightpink: Cluster 3 SE -> Excluded 


# Create a separate custom legend using ggplot2
custom_legend <- ggplot() +
  geom_point(aes(x = 1, y = 1, color = c('skyblue3', 'slateblue3', 'lightpink')),
             shape = 16, size = 4) +
  scale_color_manual(
    name = 'Cluster',
    labels = c('Included', 'Alternatively Spliced', 'Excluded'),
    values = c('skyblue', 'slateblue', 'lightpink')
  ) +
  theme_void()

# Combine the cluster plot and custom legend
plot_combined <- cowplot::plot_grid(kmeans_plot_SE_k3, custom_legend, ncol = 2, rel_widths = c(0.8, 0.2))
...
kmeans_plot_RI <- fviz_cluster(
  kmeans(histo_mat_RI, centers = 3), 
  geom = "point", 
  data = histo_mat_RI, 
