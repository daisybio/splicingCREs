...
elbowplot_SE_RI <- ggplot() +
  geom_line(elbow_SE, aes(x = k, y = tot_withinss), color='lightblue') + 
  geom_line(elbow_RI, aes(x = k, y = tot_withinss), color='lightpink') + 
  geom_point() + 
  scale_x_continuous(breaks = 1:10) + 
  labs(title='Elbow Plot SE (blue) and RI (pink)', x='Number of clusters k', y='Within groups sum of squares')
elbow
# Kmeans clustering 
...
library('factoextra')
kmeans_plot_SE <- fviz_cluster(kmeans(histo_mat_SE, centers = 3), geom = "point", data = histo_mat_SE, main='Cluster for the histogram values of each SE event', palette = c("#66C2A5", "#FC8D62", "#8DA0CB"), ellipse.type='norm', stand=TRUE, show.clust.cent=FALSE, guides=FALSE) +  #scale_color_brewer(palette='Set2', guides=FALSE)  # set guides=FALSE to remove legend

  scale_color_manual(values = c("#66C2A5", "#FC8D62", "#8DA0CB"), 
                     labels = c("Cluster A", "Cluster B", "Cluster C")) +
  theme(legend.position = "bottom")
# Rename cluster 1 to "Cluster A"
kmeans_plot_SE

kmeans_plot_SE <- fviz_cluster(kmeans_SE, geom = "point", data = histo_mat_SE,
                               main = "Cluster for the histogram values of each SE event",
                               palette = "Set2", ellipse.type = "norm") +
  labs(color = "Cluster",
       x = "Dimension 1",
       y = "Dimension 2",
