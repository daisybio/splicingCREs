...
sample_SE_2_histo <- apply(sample_SE_2, 1, function(x) {barplot(x, plot = TRUE, main="Random event of cluster 'Alternatively Spliced - SE'", xlab='Amount of Bins', width=1)}) 
dev.off()
...
sample_RI_1_histo <- apply(sample_RI_1, 1, function(x) {barplot(x, plot = TRUE, main="Random event of cluster 'Alternatively Spliced - RI'", xlab='Amount of Bin', width=1)})
dev.off()
...
col_names <- c("Included", "Alternatively Spliced", "Excluded")
...
save_kable(x=table_quantiles_RI, file= "/nfs/home/students/a.schuhe/scripts/splicingREMs/plots/quantiles_table.html")
...
lower_triangle_mat_SE <- upper_triangle_mat_SE[, .(emd, from=to, to=from)]
...
lower_triangle_mat_RI <- upper_triangle_mat_RI[, .(emd, from=to, to=from)]
...
events_filtered_SE <- events[grepl("SE", events$event_id), ]
n_bins <- 50
my_breaks <- seq(0,1,length.out=n_bins+1)
psi_mat_SE <- as.matrix(events[, ..ihecs])
dens_list_SE <- apply(psi_mat_SE, 1, function(x) {
  x <- x[!is.na(x)]
  dens <- hist(x, breaks = my_breaks, plot = FALSE)$density
  dens/sum(dens)
}, simplify = FALSE)
names(dens_list_SE) <- as.character(events_filtered_SE[, as.character(ID)])

upper_triangle_SE <- rbindlist(pbmcapply::pbmclapply(
  combn(dens_list_SE, 2, simplify = FALSE), 
  function(l) {
    list(combination = paste(names(l), collapse = "-"), emd=emdC(l[[1]], l[[2]]))
  }, mc.cores=20
  ))
upper_triangle_SE[, c('from', 'to') := tstrsplit(combination, "-", fixed=TRUE, type.convert = TRUE)]
upper_triangle_SE[, combination:=NULL] 
lower_triangle_SE <- upper_triangle_SE[, .(emd, from=to, to=from)]
diagonal_SE <- data.table(emd=0, from=events_filtered_SE[, ID], to=events_filtered_SE[, ID])
emd_mat_SE <- rbindlist(list(upper_triangle_SE, diagonal_SE, lower_triangle_SE))
View(emd_mat_SE)
dist_dt_SE <- dcast(emd_mat_SE, from ~ to, value.var = "emd")
dist <- as.dist(as.matrix(dist_dt[, -"from"]))
plot(hclust(dist))
```
