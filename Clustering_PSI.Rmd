...
diagonal_mat_RI <- data.table(emd=0, from=events[, event_id], to=events[, event_id])
...
na_per_row <- apply(events[, ..ihecs], 1, function(x) sum(is.na(x)))
events <- events[na_per_row < length(ihecs)]
events_subset_SE <- events[grep("SE", events$event_id), ]
n_bins <- 50
my_breaks <- seq(0,1,length.out=n_bins+1)
psi_mat_SE <- as.matrix(events_subset_SE[, ..ihecs])
dens_list_SE <- apply(psi_mat_SE, 1, function(x) {
  x <- x[!is.na(x)]
  dens <- hist(x, breaks = my_breaks, plot = FALSE)$density
  dens/sum(dens)
}, simplify = FALSE)
names(dens_list_SE) <- as.character(events_subset_SE[, as.character(ID)])
```
...
diagonal_SE <- data.table(emd=0, from=events_subset_SE[, ID], to=events_subset_SE[, ID])
