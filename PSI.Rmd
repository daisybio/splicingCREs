...
# remove the rows where every value is NA 
emd_matrix1 <- emd_matrix[!is.na(emd_matrix), ]
emd_matrix2 <- emd_matrix[complete.cases(emd_matrix), ] #[apply(emd_matrix, 1, function(x) !is.nan(x)), ]
dim(emd_matrix2)
View(emd_matrix2)
# rows that are completely NaN
ind <- rowSums(is.nan(emd_matrix)) == ncol(emd_matrix)
rows = which(rowSums(is.nan(emd_matrix)) == nrow(emd_matrix))
emd_matrix3 <- emd_matrix[-rows, ]
dim(emd_matrix3)
...
hcluster_emd_matrix <- hclust(emd_matrix)
