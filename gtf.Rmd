...
for (i in 1:nrow(gtf)){
  ...
  gtf$frame[i] <- '.'
  gtf$attribute[i] <- sprintf('event_id "%s" ; event_type "%s"', event_id, separated_values[2]) #gene_id and Skipped Exon or Retained Intron 
...
write.table(gtfTest, file = "annotation_events.gtf", sep = "\t", quote = FALSE, row.names = FALSE)
